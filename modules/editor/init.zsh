...
#
# Usage:
#   To enable key bindings, add the following to zzshrc, and replace 'map'
#   with 'emacs' or 'vi.
#
#     zstyle ':zsh:module:editor' keymap 'map'
#
...
if [[ "$KEYMAP" == 'vicmd' ]]; then
    zstyle -s ':zsh:module:editor:info:keymap:alternate' format 'REPLY'
    editor_info[keymap]="$REPLY"
else
    zstyle -s ':zsh:module:editor:info:keymap:primary' format 'REPLY'
    editor_info[keymap]="$REPLY"

    if [[ "$ZLE_STATE" == *overwrite* ]]; then
        zstyle -s ':zsh:module:editor:info:keymap:primary:overwrite' format 'REPLY'
        editor_info[overwrite]="$REPLY"
    else
        zstyle -s ':zsh:module:editor:info:keymap:primary:insert' format 'REPLY'
        editor_info[overwrite]="$REPLY"
    fi
fi
...
local indicator
zstyle -s ':zsh:module:editor:info:completing' format 'indicator'
print -Pn "$indicator"
...
do
    ...
    # Expand .... to ../..
    if zstyle -t ':zsh:module:editor' dot-expansion; then
...
# Set the key layout.
zstyle -s ':zsh:module:editor' keymap 'keymap'
if [[ "$keymap" == (emacs|) ]]; then
    bindkey -e
elif [[ "$keymap" == vi ]]; then
    bindkey -v
else
    print "zsh: invalid keymap: $keymap" >&2
fi
